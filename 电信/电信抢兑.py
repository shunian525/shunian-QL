...
def send(uid, content):
    r = requests.post('https://wxpusher.zjiecode.com/api/send/message',
                      json={"appToken": "AT_3hr0wdZn5QzPNBbpTHFXawoDIsSUmPkN", "content": content, "contentType": 1,
                            "uids": [uid]}).json()
    return r
    ...
    try:
        r = ss.post('https://appgologin.189.cn:9031/login/client/userLoginNormal', json={
            "headerInfos": {"code": "userLoginNormal", "timestamp": timestamp, "broadAccount": "", "broadToken": "",
                            ...
                            "sourcePassword": "Sid98s", "token": "", "userLoginName": phone},
            "content": {"attach": "test", "fieldData": {"loginType": "4", "accountType": "",
                                                        ...
                                                        "systemVersion": "15.4.0", "authentication": password}}}).json()

        printn(f"登录响应: {r}")  # 打印完整的响应以便调试

        if 'responseData' not in r or 'data' not in r['responseData'] or 'loginSuccessResult' not in r['responseData'][
            'data']:
...
def queryInfo(phone, s):
    ...
    while a < 10:
        ...
        if amountTotal < 3000:
            ...
            res = s.post('http://wapact.189.cn:9000/gateway/standExchange/detail/exchange', json={"activityId": jdaid},
                         cookies=ck).text
    ...
    for cishu in range(3):
        try:
            if rs:
                ...
                cookie = {}
            r = s.post('https://wapact.189.cn:9001/gateway/golden/api/lottery',
                       json={"activityId": "6384b49b1e44396da4f1e4a3"}, cookies=ck)
        except:
            pass
        time.sleep(3)
...
def first_request(res=''):
    global js, fw
    url = 'https://wapact.189.cn:9001/gateway/standExchange/detail/exchange'
    if res == '':
        response = ss.get(url)
        res = response.text
